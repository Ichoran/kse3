...
object JsonCharBufferParser extends JsonGenericParserCompanion.Positional[CharBuffer, JsonCharBufferParser] {
  ...
  protected inline def whiteless(in: CharBuffer): Int =
    var c = -129
    while (
  ...
  def parse(input: CharBuffer, options: JsonOptions = JsonOptions.Default): Jast = parseImpl(input, options)

  def parseJson(input: CharBuffer, options: JsonOptions = JsonOptions.Default): kse.jsonal.Json | JastError = parseJsonImpl(input, options)

  def parseNull(input: CharBuffer, options: JsonOptions = JsonOptions.Default): kse.jsonal.Json.Null.type | JastError = parseNullImpl(input, options)
  
  def parseBool(input: CharBuffer, options: JsonOptions = JsonOptions.Default): kse.jsonal.Json.Bool | JastError = parseBoolImpl(input, options)
  
  def parseStr(input: CharBuffer, options: JsonOptions = JsonOptions.Default): kse.jsonal.Json.Str | JastError = parseStrImpl(input, options)

  def parseNum(input: CharBuffer, options: JsonOptions = JsonOptions.Default): kse.jsonal.Json.Num | JastError = parseNumImpl(input, options)
  
  def parseArr(input: CharBuffer, options: JsonOptions = JsonOptions.Default): kse.jsonal.Json.Arr | JastError = parseArrImpl(input, options)
  
  def parseObj(input: CharBuffer, options: JsonOptions = JsonOptions.Default): kse.jsonal.Json.Obj | JastError = parseObjImpl(input, options)
}
