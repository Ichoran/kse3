...
final class JsonRecyclingParser extends RecyclingBuffer {
  ...
  def start = i0
  def start_=(i: Int): Unit = { i0 = if (i < 0) 0 else if (i > iN) iN else i }
  def end = iN
  def end_=(i: Int): Unit = {
  ...
  def offset = o
  def offset_=(l: Long): Unit = { if (l < 0) o = 0 else o = l }
  def source = src
  def source_=(s: RecyclingBuffer => Boolean): Unit = { src = s }
  def exhausted = done
  def exhausted_=(b: Boolean): Unit = { done = b }
  def buffer = a
  def buffer_=(b: Array[Byte]): Unit = { a = b; if (iN > b.length) iN = b.length }
  def pack(): this.type = {
    if (i0 > 0) {
      o += i0
      if (iN > i0) {
